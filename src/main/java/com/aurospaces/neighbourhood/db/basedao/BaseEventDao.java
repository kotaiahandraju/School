...
@Autowired public JdbcTemplate jdbcTemplate;
	...
	public final String INSERT_SQL = "INSERT INTO notification( created_time, updated_time, studentId,  senderId,message,notificationId) values ( ?, ?, ?, ?,?,?)"; 
...
ps.setString(6, objAttendanceBean.getNotificationId());
		...
		else
		{
			...
			String sql = "UPDATE notification  set studentId = ? ,senderId = ?  ,senderId = ? ,message = ? ,notificationId = ? where id = ? ";
	
			jdbcTemplate.update(sql, new Object[]{objAttendanceBean.getStudentId(),objAttendanceBean.getSenderId(),objAttendanceBean.getMessage(),objAttendanceBean.getNotificationId(),objAttendanceBean.getId()});
		}
	}
