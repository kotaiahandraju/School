...
public class ExamPatternDao {
	...
	public final String INSERT_SQL = "INSERT INTO exampattern( createdTime, updatedTime, boardId, classId, subjectId, examtypeId,sectionId, maxMarks,mediumId) values ( ?, ?, ?, ?, ?, ?, ?,?,?)"; 
		...
		if(examPatternBean.getId() == 0)	{
			...
			int update = jdbcTemplate.update(
				...
				else
				{
					...
					String sql = "UPDATE exampattern  set updatedTime = ? ,boardId=?, classId=?, subjectId=?, examtypeId=?,sectionId=?, maxMarks=?, mediumId=?  where id = ? ";
	...
	public List<Map<String, String>> getExamPatternList(String boardId, String classId,String sectionId, String mediamId, String examtypeId) {
			...
			objStringBuffer
					.append("select ep.id,b.name as boardName,ep.boardId as boardId,cl.name as className,ep.classId as classId,sec.name as sectionName," 
							 ...
							 + "sub.name as subjectName,ep.subjectId as subjectId,ep.maxMarks as maxMarks from " 
							 + "exampattern ep , mediam m,sectiontable sec, classtable cl,boardname b,subject sub, exam_type et"
							 + " where ep.boardId=b.id and ep.sectionId=sec.id and ep.classId=cl.id and ep.mediumId=m.id and ep.examTypeId=et.id and ep.subjectId=sub.id ");
...
*/
			...
			RowValueCallbackHandler handler = new RowValueCallbackHandler(
					new String[] {"id", "boardName","boardId","classId","className","sectionId", "sectionName","mediam","mediumId","examType","examTypeId",  
							"subjectName","subjectId","maxMarks"});
